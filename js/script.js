...
$('#series').val('');
$('#make').val('');
$('#add_vehicle').removeClass();
$('#add_vehicle').prop('type', 'button');
$('#add_vehicle').addClass('btn btn-md disabled').fadeIn();
...
$('#otherseries').val('');  
$('#add_vehicle').removeClass();
$('#add_vehicle').prop('type', 'submit');
$('#add_vehicle').addClass('btn btn-md').fadeIn();
};
});
...
$('#plateNumber1').on('keyup', function(){
  var plateNumber1 = $('#plateNumber1').val();
  if (plateNumber1 == '') {
    plateNumber1_state = false;
    return;
  }
  $.ajax({
      url: 'vehiclesinfo.php',
      type: 'post',
      data: {
        'plateNumber1_check' : 1,
        'plateNumber1' : plateNumber1,
      },
      success: function(response){
        if (response == 'taken' ) {
          $('#plateNumber1').parent().removeClass();
          $('#plateNumber1').css("border","1px solid #D83D5A"); 
          $('#plateNumber1').siblings("span").css("color","#D83D5A");
          $('#plateNumber1').siblings("span").text('Plate Number already exist').fadeIn();
          $('#add_vehicle').removeClass();
          $('#add_vehicle').prop('type', 'button');
          $('#add_vehicle').addClass('btn btn-md disabled').fadeIn();
          return;
          ...
          $('#plateNumber1').parent().removeClass();
          $('#plateNumber1').css("border","1px solid green"); 
          $('#plateNumber1').siblings("span").css("color","green");
          $('#plateNumber1').siblings("span").text('Plate number available').fadeIn();
          return;
        }
      }
  });
...
$('#make').blur(function() {
  var makepattern = new RegExp(/\s/g, '');
  var makepattern1 = new RegExp(/[A-Za-z]{1,20}/i);
  if (this.value == '' || makepattern.test($("#make").val())) {
...
$('#firstName').blur(function() {
  var firstNamepattern = new RegExp(/\s/g, '');
  var firstNamepattern1 = new RegExp(/[A-Za-z]{1,20}/i);
  if (this.value == '' || firstNamepattern.test($("#firstName").val())) {
...
$('#middleName').blur(function() {
  var middleNamepattern = new RegExp(/\s/g, '');
  if (this.value == '' || middleNamepattern.test($("#middleName").val())) { 
...
$('#lastName').blur(function() {
  var lastNamepattern = new RegExp(/\s/g, '');
  if (this.value == '' || lastNamepattern.test($("#lastName").val())) {
...
$('#address').blur(function() {
  var addresspattern = new RegExp(/\s/g, '');
  if (this.value == '' ) {
...
$('#contactNumber').blur(function() {
  var contactNumberpattern = new RegExp(/\s/g, '');
  if (this.value == '' || contactNumberpattern.test($("#contactNumber").val())) {
...
$('#email').blur(function() {
  var emailpattern = new RegExp(/\s/g, '');
  if (this.value == '' || emailpattern.test($("#email").val())) {
    $('#email').parent().removeClass();
...
$('#username').blur(function() {
  var usernamepattern = new RegExp(/\s/g, '');
  if (this.value == '' || usernamepattern.test($("#username").val())) {
...
$('#password').blur(function() {
  var passwordpattern = new RegExp(/\s/g, '');
  if (this.value == '' || passwordpattern.test($("#password").val())) {
