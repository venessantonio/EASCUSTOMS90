<?php
	...
	$month = $_POST["date"];
    $year = $_POST["date2"];
	$sql = "SELECT chargeinvoice.date as cDate, plateNumber, make, series,TotalPrice from chargeinvoice join appointments on chargeinvoice.appointmentId = appointments.id join vehicles on appointments.vehicleId = vehicles.id where month(chargeinvoice.date) = '".$month."' AND year(chargeinvoice.date) = '".$year."';";
...
$pdf = new FPDF('P','mm','A4');
	...
	endforeach;
	$query = "SELECT sum(TotalPrice) as total FROM chargeinvoice where month(date) = '".$month."' AND year(date) = '".$year."';";
