...
$dates = $date;
  ...
  if($action=='accept'){
    $actions_command = $connection->prepare("UPDATE `appointments` SET `status` = 'Accepted', `modified` = now(), `color` = '#4caf50', `date` = '$dates',`targetEndDate` =  '$dates', `notification` = 0 WHERE `appointments`.`id` = $id;");
  }else{
    if($action=='deny'){
...
if(isset($_POST["update_spareparts"])){
  ...
  $query = $connection->prepare("UPDATE `spareparts` SET `name`=?,`price`=?,`brandName`=?,`description`=?,`status`=? WHERE id = ?");
   $query->bind_param('sssssi',$name, $price, $brand, $desc, $status, $id);
  if($query->execute()){
    header("Location: ../sparepartsmanagement.php");
  }else{
    header("Location: ../error.php");
